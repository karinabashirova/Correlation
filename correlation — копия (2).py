...
if __name__ == '__main__':
    ...
    # fig = go.Figure()
    # fig.add_trace(
    #     go.Scatter(x=reader.time[0], y=reader.spot[0], mode="lines"))
    ...
    for k in range(len(volatility)):
        ...
        for i in range(len(volatility[k])):
            tmp_corr.append(np.corrcoef(reader.spot[k], volatility[k][i])[0, 1])
        ...
        for i in indexes:
            tmp_corr_spearman.append(spearmanr(reader.spot[k], volatility[k][i])[0])

        # print(tmp_corr_spearman)

        # fig1 = go.Figure()
        # fig1.add_trace(go.Scatter(x=[1]*len(reader.spot[0]), y=reader.spot[0], mode="markers"))
        # fig1.add_trace(go.Scatter(x=[2]*len(reader.spot[1]), y=reader.spot[1], mode="markers"))
        # fig1.show()

        print('TIME', reader.time[k])
        print('SPOT', reader.spot[k])
        print()
        ...
        try:
            if len(tmp) > 1:
                ...
                for i, index in zip(np.arange(len(indexes)), indexes):
                    fig[k].add_trace(go.Scatter(x=reader.spot[k], y=volatility[k][index], mode="markers", marker=dict(color=reader.time[k])), row=i + 1, col=1)
            ...
            if len(tmp) == 1:
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=reader.spot[k], y=volatility[k][0], mode="markers",marker=dict(color=reader.time[k]),
                                         name="t " + str(reader.time[k][0]*60*60)), row=i + 1, col=1)
