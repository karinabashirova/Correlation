...
create_table = async (iname) => {
    var result = ",k,bid_c,ask_c,bid_p,ask_p,e,q,qty_p,qty_c,s0\n";
    ...
    if (result == ",k,bid_c,ask_c,bid_p,ask_p,e,q,qty_p,qty_c,s0\n")
      {
    ...
    //console.log(result);
    const logFile = "./bit/bit_" + moment.utc().format("YYYY-MM-DD.HH-mm-ss") + ".csv";
    console.log(logFile);
    fs.writeFileSync(logFile, result);
    //fs.writeFileSync(__dirname + "\\bit\\bit_" + (time.replace(':', '-')) + ".csv", result)
    return;
}
get_deribit_prices_mm = () => {
    ...
    if (process.argv.length < 4)
    {
        ...
        console.log("usage: node get_all_bit_prices_mm.js <period in sec (0: without timer)> <u_token-DATE>");
        return;
    }
    ...
    let period = process.argv[2];
    let iname = process.argv[3];
    let k = 4;
